  ...
  if (cart.length < 1) {
    return (
      <Stack gap="15px" p="20px" align="center">
        ...
        <Text>
          Tu carrito de compras está vacio, dale un vistazo a nuestros{" "}
          <Link to="/">
            <Button variant={"link"} color={"#7A26C1"}>
              productos
            </Button>
          </Link>
        </Text>
        <Link to="/checkout">
          <Button color={"#7A26C1"}>Última órden</Button>
        </Link>
      </Stack>
    );
  }
      ...
      <Modal isOpen={isOpen} onClose={onClose} isCentered>
        ...
        <ModalContent>
          ...
          <ModalBody>
            <Text>
              {" "}
              Si borras tu carrito no podrás recuperarlo, si quieres eliminar un
              item en particular borralos desde su boton de borrar:{" "}
              <DeleteIcon />
            </Text>
          </ModalBody>
